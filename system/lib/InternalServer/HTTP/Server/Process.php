<?php
/* The begining of a daemonised web server based off Nanoweb */
/*
 * Nanoweb, the aEGiS PHP web server ================================= Copyright
 * (C) 2002-2005 Vincent Negrier aka. sIX <six@aegis-corp.org> This program is
...
class Process {
	...
	const DEFAULT_LISTEN_QUEUE = 20;
	const DEFAULT_MIMETYPES = "/etc/mime.types";
	const DEFAULT_CONTENT_TYPE = "text/plain";
	const DEFAULT_REQUEST_TIMEOUT = 15;
	const DEFAULT_ACCESS_FILE = ".nwaccess";
	const DEFAULT_DOCROOT = "./";
	const DEFAULT_LOGFILE = "./access.log";
	const DEFAULT_LOGTYPE = "combined";
	const DEFAULT_FBWELCOMEFILE = ".welcome";
	const DEFAULT_STATIC_BUFFER_SIZE = 1048576;
	const DEFAULT_CONTENT_HANDLER = "static";
	const DEFAULT_MAX_SERVERS = 25;
	const DEFAULT_SERVER_LANG = "en-us";
	
	const SCK_WRITE_PACKET_SIZE = 8192;
	const SCK_READ_PACKET_SIZE = 4096;
	const SCK_READ_SELECT_TIMEOUT = 2;
	const SCK_MAX_STALL_TIME = 60;
	
	const SPM_CACHES_LIFETIME = 15;
	
	const HTTP_VERSION = "HTTP/1.1";
	
	const SERVER_STRING = "CoreLib2_Server";
	
	function SERVER_STRING_V() {
		return static::SERVER_STRING . "/" . static::VERSION;
	}
	
	const INT_MSGSIZE = 4096;
	
	const NM_HIT = "  HIT";
	const NM_RESTART_LOGGERS = "LGRST";
	const NM_SERVER_STATE = "SRVST";
	const NM_RELOAD_THEME = "RLTHM";
	const NM_BLOCK_IP = "BANIP";
	const NM_UNBLOCK_IP = "DBNIP";
	
	const NW_BAD_OUTSIDE_DOCROOT = 1;
	const NW_BAD_DOT_FILE = 2;
	const NW_BAD_WIN_DEVICE = 3;
	
	const NW_SB_STATUS = 0;
	const NW_SB_PEERHOST = 1;
	const NW_SB_FORKTIME = 2;
	
	const NW_EL_DEBUG = 1;
	const NW_EL_HIT = 2;
	const NW_EL_NOTICE = 4;
	const NW_EL_BLOCKING = 8;
	const NW_EL_WARNING = 16;
	const NW_EL_ERROR = 32;
	const NW_EL_ALL = 255;
	/*
	 * const NW_EL_DEFAULT = NW_EL_NOTICE | NW_EL_BLOCKING | NW_EL_WARNING |
	 * NW_EL_ERROR;
	 */
	
	const NW_TMPL_SIGNATURE = "server_signature";
	const NW_TMPL_ERROR_PAGE = "error_page";
	const NW_TMPL_ERROR_RESOURCE = "error_resource";
	const NW_TMPL_ERROR_ADMIN = "error_admin";
	
	const REQUIRED_PHP_VERSION = "4.2.0";
	...
	static $conf_defaults = array (
			"listeninterface" => DEFAULT_LISTEN_ADDR,
	...
	static $conf_vhosts_propagate = array (
			"documentroot",
			"directoryindex",
			"serveradmin",
			"user",
	...
	/*
	 * static $cmdline_help = <<<EOT Usage: nanoweb.php [/path/to/nanoweb.conf]
	 * [options] nanoweb supports the following command line options : --help -h
	 * : this help screen --version -v : show version info
	 ...
	 * --start-daemon -d : start nanoweb and run in background --config-test -t
	 * : test configuration and exit --quiet -q : don't send text to console
	 * EOT;
	 */
		...
		if (! $conf [$cpart] ["_nw_pcp"]) {
			...
			if (! $nodefaults) {
				...
				if ($cpart == "global") {
					...
					foreach ( $GLOBALS ["conf_defaults"] as $key => $dval )
						if (! isset ( $conf [$cpart] [$key] ))
							$conf [$cpart] [$key] [0] = $dval;
				
				} else {
					
					// Propagate appropriate directives to vhosts
					
					foreach ( $GLOBALS ["conf_vhosts_propagate"] as $key => $pkey )
						if (! isset ( $conf [$cpart] [$pkey] ))
							$conf [$cpart] [$pkey] = $conf ["global"] [$pkey];
					if (! $conf [$cpart] ["servername"])
						$conf [$cpart] ["servername"] [] = $cpart;
	...
	function parseconfig($conf_arr, $nodefaults = false) {
					...
					default :
						...
						switch ($cnfk) {
							...
							case "documentroot" :
								...
								if (substr ( $rp, - 1 ) != DIRECTORY_SEPARATOR)
							...
							case "serverlog" :
								...
								if ($lmode = strtok ( "" )) {
									...
									$lbmode = 0;
									foreach ( $GLOBALS ["srvlog_levels"] as $lvl => $bin_lvl )
										if (strpos ( $lmode, $lvl ) !== false)
											$lbmode |= $bin_lvl;
									foreach ( $GLOBALS ["srvlog_levels"] as $lvl => $bin_lvl )
										if (strpos ( $lmode, "-" . $lvl ) !== false)
											$lbmode &= ~ $bin_lvl;
		...
		foreach ( $_pcp as $clpart )
			_parseconfig_close_part ( $conf, $clpart, $nodefaults );
	...
	function cmdline_conf_upd($conf, $cmdline_conf_overrides, $cmdline_conf_adds) {
		...
		foreach ( $cmdline_conf_overrides as $cs ) {
			...
			$ca = explode ( "=", $cs );
			$conf ["global"] [strtolower ( $ca [0] )] = array (
					$ca [1] 
			);
	...
	function load_modules($conf) {
		...
		if ($lm_arr = $conf ["global"] ["loadmodule"])
			foreach ( $lm_arr as $key => $modname ) {
				
				$clsname = basename ( $modname, ".php" );
				
				if (! is_file ( $modname )) {
					
					foreach ( access_query ( "modulesdir" ) as $md )
						if (is_file ( $md . DIRECTORY_SEPARATOR . $modname )) {
				...
				if (! $ld_clss [$clsname]) {
					...
					if ($modloaded) {
						...
						if (is_array ( $tmp->urls ))
							foreach ( $tmp->urls as $url )
								$modules ["url:" . $url] = &$tmp;
						if (is_array ( $tmp->methods ))
							foreach ( $tmp->methods as $method )
								$modules ["method:" . $method] = &$tmp;
						if (is_string ( $mt = $tmp->sig_token ))
							$mod_tokens [$tmp->modname] = $mt;
						
						if ($nload)
							techo ( "loaded module : " . $tmp->modname );
	...
	function load_theme($themefname, $load_notice = false, $reload = false) {
		...
		if ($thmarr = @file ( $tfn )) {
			...
			if ($theme ["theme_id"]) {
				...
				if (! $theme ["theme_name"])
					$theme ["theme_name"] = $theme ["theme_id"];
				$theme ["theme_id"] = trim ( $theme ["theme_id"] );
				$theme ["theme_name"] = trim ( $theme ["theme_name"] );
				
				$theme ["theme_language"] = trim ( $theme ["theme_language"] ) or $theme ["theme_language"] = DEFAULT_SERVER_LANG;
				
				if ($load_notice)
					techo ( ($reload ? "re" : "") . "loaded theme : " . $theme ["theme_name"] . " (" . $ts . " bytes)" );
	...
	function load_themes($conf) {
		...
		if (is_array ( $conf ["global"] ["loadtheme"] ))
			foreach ( $conf ["global"] ["loadtheme"] as $themefname ) {
	...
	function load_access_files($dir, &$access, $rec = 0) {
		...
		if (is_array ( $z = $access_cache [$dir] )) {
			...
			$cont_dirs = array (
					nw_realpath ( $GLOBALS ["docroot"] ) 
			);
			foreach ( $conf [$GLOBALS ["vhost"]] ["allowsymlinkto"] as $als )
				$cont_dirs [] = nw_realpath ( $als );
			
			foreach ( $cont_dirs as $cdir )
				if (strpos ( $r_mdir, $cdir ) === 0) {
			...
			if (is_readable ( $afn ) && ($accesstmp = @file ( $afn )) && ($tmp_access = parseconfig ( $accesstmp, true )))
				foreach ( $tmp_access as $key => $val_arr ) {
					
					foreach ( $val_arr as $ckey => $cval_arr ) {
						
						$ap = $GLOBALS ["access_policy"] [$ckey] or $ap = $conf ["global"] ["accesspolicy"] [0];
	...
	function access_query($key, $idx = false) {
		...
		$ap = $GLOBALS ["access_policy"] [$key] or $ap = $conf ["global"] ["accesspolicy"] [0];
		
		switch ($ap) {
			
			case "override" :
				$tmp = $access ["global"] [$key] or $tmp = $conf [$GLOBALS ["vhost"]] [$key] or $tmp = $conf ["global"] [$key];
				break;
	...
	function core_modules_hook($hname) {
		...
		if ($mh_arr = &$GLOBALS ["modules"] ["core_" . $hname])
			foreach ( array_keys ( $mh_arr ) as $a )
				$mh_arr [$a]->main ();
	...
	function log_ids() {
		...
		if ($setgid = @posix_getgrnam ( $conf ["global"] ["loggergroup"] [0] ))
			$g = $setgid ["gid"];
		else if ($setgid = @posix_getgrnam ( $conf ["global"] ["group"] [0] ))
			$g = $setgid ["gid"];
		
		if ($setuid = @posix_getpwnam ( $conf ["global"] ["loggeruser"] [0] ))
			$u = $setuid ["uid"];
		else if ($setuid = @posix_getpwnam ( $conf ["global"] ["user"] [0] ))
			$u = $setuid ["uid"];
	...
	function log_srv($str, $loglevel = NW_EL_NOTICE) {
		...
		if ($srvlog_arr = $GLOBALS ["conf"] ["global"] ["_serverlog"])
			foreach ( $srvlog_arr as $s => $bmode )
				if ($loglevel & $bmode) {
					
					if (($GLOBALS ["pmode"] == "master") && (! file_exists ( $s )))
						$chown = true;
	...
	function nw_server_string() {
		...
		switch (strtolower ( access_query ( "serversignature", 0 ) )) {
			...
			case "fake" :
				return (access_query ( "serverfakesignature", 0 ));
			case "off" :
				return ("");
			case "prod" :
				return (SERVER_STRING);
			case "min" :
				return (SERVER_STRING_V);
			case "os" :
				return (SERVER_STRING_V . " (" . PHP_OS . ")");
			case "php" :
				return (SERVER_STRING_V . " (" . PHP_OS . "; PHP/" . phpversion () . ")");
	...
	function _genpage_signature() {
		...
		return (nw_apply_template ( NW_TMPL_SIGNATURE, array (
				"server_string" => nw_server_string (),
				"server_name" => $GLOBALS ["conf"] [$GLOBALS ["vhost"]] ["servername"] [0] 
		), true ));
	...
	function nw_server_vars($include_cgi_vars = false) {
		...
		if ($asv = access_query ( "addservervar" ))
			foreach ( $asv as $str ) {
		...
		if ($GLOBALS ["add_nsv"])
			foreach ( $GLOBALS ["add_nsv"] as $key => $val )
				$nsv [$key] = $val;
		
		if ($include_cgi_vars && ($rq_hdrs = $GLOBALS ["htreq_headers"]))
			foreach ( $rq_hdrs as $key => $val )
				$nsv ["HTTP_" . str_replace ( "-", "_", $key )] = $val;
	...
	function nw_host_to_vhost($host, $lport = 80) {
		...
		if (is_array ( $conf [$phost = ($host . ":" . $lport)] ))
			return ($phost);
		...
		for($vhlen = 0; $vhlen <= $hlen; $vhlen ++) {
			...
			if (is_array ( $conf [$phost = ($whost . ":" . $lport)] ))
				return ($phost);
			if (is_array ( $conf [$whost] ))
				return ($whost);
	...
	function nw_error_page($rq_err) {
		...
		$err ["error_resource"] = ($http_resource ? (nw_apply_template ( NW_TMPL_ERROR_RESOURCE, array (
				"resource_name" => htmlentities ( $http_resource ) 
		), true )) : "");
		if ($conf [$vhost] ["serveradmin"] [0])
			$err ["error_admin"] = nw_apply_template ( NW_TMPL_ERROR_ADMIN, array (
					"admin" => $conf [$vhost] ["serveradmin"] [0] 
			), true );
	...
	function nw_allow_list($ext) {
		...
		foreach ( $GLOBALS ["modules"] as $tmpmod )
			if (method_exists ( $tmpmod, "options" ))
				if ($mod_methods = $tmpmod->options ())
					foreach ( $mod_methods as $mod_method )
						if (! isset ( $tmp_marr [$mod_method] ))
							$tmp_marr [$mod_method] = $mod_method;
	...
	function read_request(&$sck_connected, &$dp, &$pn, $maxlen = 0) {
		...
		while ( ! $rq_finished && $sck_connected ) {
			...
			if (! $tnreq) {
				...
				if ($conf ["global"] ["servermode"] [0] == "inetd") {
					...
					if (feof ( $GLOBALS ["inetd_in"] )) {
						...
						$tmp = fgetc ( $GLOBALS ["inetd_in"] );
				
				} else {
					
					if ($ns = socket_select ( $fdset, $write = NULL, $fdset, SCK_READ_SELECT_TIMEOUT ))
			...
			} else if (($ns) || ((! $ns) && ((time () - $wstart) >= $conf ["global"] ["requesttimeout"] [0])))
				$sck_connected = false;
	...
	function send_response($response, &$sck_connected) {
		...
		while ( $sent_len < $resp_len && $sck_connected ) {
			...
			if ($GLOBALS ["conf"] ["global"] ["servermode"] [0] == "inetd") {
				...
				if (($sent_len + SCK_WRITE_PACKET_SIZE) > $resp_len)
					$size = $resp_len - $sent_len;
				else
					$size = SCK_WRITE_PACKET_SIZE;
				if (($ret = @socket_write ( $msgsock, substr ( $response, $sent_len, $size ), $size )) > 0)
					$sent_len += $ret;
				...
				if ($ret === false) {
					...
					if (socket_last_error ( $msgsock ) == SOCKET_EWOULDBLOCK) {
						...
						if (! socket_select ( $read = NULL, $fdset, $except = NULL, SCK_MAX_STALL_TIME ))
							$sck_connected = false;
	...
	function nw_block_ip_address($ip_addr, $type, $source, $expires = 0) {
		...
		if ((($conf ["global"] ["singleprocessmode"] [0]) || ($pmode == "master")) && (! $banned_ips [$ip_addr])) {
			...
			int_sendtomaster ( NM_BLOCK_IP, array (
					$ip_addr,
					$type,
					$source,
					$expires 
	...
	function __construct() {
		...
		if (version_compare ( phpversion (), REQUIRED_PHP_VERSION ) < 0)
			errexit ( "nanoweb needs PHP >= " . REQUIRED_PHP_VERSION . " (you are using " . phpversion () . ")" );
		...
		foreach ( $TEST_FUNCS as $f_name => $f_mandatory )
			if (! is_callable ( $f_name )) {
				
				if ($f_mandatory)
					errexit ( "function '" . $f_name . "' not available, aborting" );
				else
					techo ( "WARN: function '" . $f_name . "' not available", NW_EL_WARNING );
		...
		if (! is_readable ( $conffile ))
			errexit ( "unable to read configuration (" . $conffile . "), aborting" );
		...
		if ($conf ["global"] ["servermode"] [0] != "inetd") {
			...
			foreach ( $conf ["global"] ["listenport"] as $lport ) {
				...
				if (($sock = @socket_create ( AF_INET, SOCK_STREAM, 0 )) < 0)
					errexit ( "socket create failed : " . socket_strerror ( socket_last_error () ) );
				...
				if (is_callable ( $setsockopt ) && is_callable ( $getsockopt )) {
					...
					if ($sbuf < (SCK_WRITE_PACKET_SIZE * 32))
						$setsockopt ( $sock, SOL_SOCKET, SO_SNDBUF, SCK_WRITE_PACKET_SIZE * 32 );
					if ($rbuf < (SCK_READ_PACKET_SIZE * 32))
						$setsockopt ( $sock, SOL_SOCKET, SO_RCVBUF, SCK_READ_PACKET_SIZE * 32 );
				
				}
				
				if (! @socket_bind ( $sock, $conf ["global"] ["listeninterface"] [0], $lport ))
					errexit ( "socket bind failed on port " . $lport . " : " . socket_strerror ( socket_last_error ( $sock ) ) );
				if (! @socket_listen ( $sock, $conf ["global"] ["listenqueue"] [0] ))
					errexit ( "socket listen failed on port " . $lport . " : " . socket_strerror ( socket_last_error ( $sock ) ) );
		...
		if ($start_daemon) {
			...
			if (! $posix_av || ! $pcntl_av)
				errexit ( "posix and pcntl PHP extensions are needed for --start-daemon" );
		...
		if ($conf ["global"] ["servermode"] [0] != "inetd")
			techo ( "ready and accepting connections" );
		...
		while ( true ) {
			...
			while ( ! $cnx ) {
					...
					foreach ( $lnset as $lnact ) {
						...
						if ($lnact == $children_sck) {
							...
							while ( $msg = socket_read ( $children_sck, INT_MSGSIZE ) ) {
								...
								if (strlen ( $msg ) > 5)
									$mcontent = unserialize ( substr ( $msg, 5 ) );
								
								switch ($mtype) {
									
									case NM_HIT :
										
										// content : 0 => pid, 1 => request
										// status, 2 => length, 3 => vhost
									...
									case NM_SERVER_STATE :
										...
										if ($scoreboard [$spid = $mcontent [0]]) {
											...
											if ($mcontent [2])
												$scoreboard [$spid] [NW_SB_PEERHOST] = $mcontent [2];
									...
									case NM_BLOCK_IP :
										...
										// content : 0 => ip address, 1 => type
										// (PERM|TEMP), 2 => source, 3 =>
										// expiration
										
										if (is_array ( $mcontent ))
											nw_block_ip_address ( $mcontent [0], $mcontent [1], $mcontent [2], $mcontent [3] );
									...
									case NM_UNBLOCK_IP :
										...
										if ($mcontent)
											nw_unblock_ip_address ( $mcontent );
							...
							if ((($active_servers < $conf ["global"] ["maxservers"] [0]) || (! $conf ["global"] ["maxservers"] [0])) && (is_resource ( $msgsock = @socket_accept ( $sock ) ))) {
								...
								if (is_array ( $banned_ips [$remote_ip] )) {
									...
									// Cannot obtain peer IP address, something
									// is wrong (but not worth throwing a
									// notice)
				...
				if (! $conf ["global"] ["singleprocessmode"] [0])
					while ( ($deadpid = pcntl_waitpid ( - 1, $cstat, WNOHANG )) && ($deadpid != - 1) ) {
						
						// Dead child
						
						if (($dead_logger = $logger_pids [$deadpid]) && ($conf ["global"] ["loggerprocess"] [0])) {
							
							// Dead logger (this is abnormal, we have to restart
							// one)
				...
				$ct = time ();
				foreach ( $banned_ips as $ip_addr => $bip )
					if (($bip ["type"] == "TEMP") && ($bip ["expires"] <= $ct))
						nw_unblock_ip_address ( $ip_addr, "expired" );
			...
			if ($conf ["global"] ["singleprocessmode"] [0]) {
				...
				// Invalidate access and rp caches every SPM_CACHES_LIFETIME
				// connections
			...
			if ($pid === 0) {
				...
				while ( $cnx ) {
					...
					if ($sck_connected) {
						...
						if ($posix_av) {
							...
							if ($uid_set) {
								...
								if ($uid_set != $ugtok) {
									...
									// Keep-alive request for another user/group
									// vhost, this is bad
								...
								if ($setgid = $sysgrp [$cfgid] ["gid"])
									posix_setgid ( $setgid );
								if ($setuid = $sysusr [$cfuid] ["uid"])
									posix_setuid ( $setuid );
						...
						if ($p1 = $http_resource) {
							...
							foreach ( access_query ( "_aliases" ) as $key => $val )
								if (strpos ( rtrim ( $http_uri, "/" ), rtrim ( $key, "/" ) ) === 0) {
									...
									if ((is_dir ( $docroot . $http_uri )) && (substr ( $docroot . $http_uri, - 1 ) != "/"))
								...
								if (($udadd = $conf [$vhost] ["userdir"] [0]) && ($posix_av)) {
									...
									if ($udinf = @posix_getpwnam ( $udname )) {
										...
										if (is_dir ( $tmpdr )) {
											...
											// User exists but does not have a
							...
							if (access_query ( "allowpathinfo", 0 ) && ! file_exists ( $docroot . $http_uri )) {
								...
								while ( ! @is_file ( $docroot . $new_uri ) && $new_uri ) {
									...
									if (! @is_file ( $docroot . $new_uri ) && $pie_arr = access_query ( "pathinfotryext" ))
										foreach ( $pie_arr as $pie_ext )
											if (@is_file ( $docroot . $new_uri . "." . $pie_ext )) {
						...
						if (($bad_rq == NW_BAD_OUTSIDE_DOCROOT) && ($als_arr = $conf [$vhost] ["allowsymlinkto"])) {
							...
							// Test for outside-docroot access exemptions
							// (AllowSymlinkTo)
							...
							while ( $tdir ) {
								...
								if ((is_link ( $docroot . $tdir )) && (strpos ( nw_realpath ( dirname ( $docroot . $tdir ) ), nw_realpath ( $docroot ) ) === 0))
									foreach ( $als_arr as $als )
										if (strpos ( nw_realpath ( $docroot . $http_uri ), nw_realpath ( $als ) ) === 0) {
						...
						if ($authls = access_query ( "authlocation" )) {
							...
							foreach ( $authls as $authl )
								if (strpos ( "/" . $real_uri, $authl ) === 0) {
						...
						if (($rauths = access_query ( "authrequire" )) && (! $bypass_auth)) {
							...
							if ($logged_user === "") {
								...
								if ($emsg = access_query ( "authmessage", 0 ))
									$add_errmsg .= $emsg . "<br><br>";
						...
						if ($htreq_headers ["CONTENT-LENGTH"]) {
							...
							// Read request content if there is one (POST
							// requests)
						...
						if ($sck_connected) {
								...
								case "HEAD" :
									...
									if ($pri_err) {
										...
										if ($rq_err < 300) {
											...
											$rq_err = $pri_redir_code or $rq_err = 302;
										
										}
										
										$out_add_headers ["Location"] = $pri_redir;
										if (version_compare ( $http_version, "1.0" ) <= 0)
											$out_add_headers ["URI"] = $pri_redir;
							...
							if ($conf ["global"] ["loggerprocess"] [0]) {
								...
								$log_arr = array (
										...
										$rq_err,
										$sent_content_length,
										$htreq_headers ["REFERER"],
										$htreq_headers ["USER-AGENT"] 
								);
								...
								if ($msglen > INT_MSGSIZE) {
									...
									if ($r != $msglen)
										techo ( "WARN: unable to communicate with logger process", NW_EL_WARNING );
								...
								if ($nb_loggers = count ( $modules ["log"] )) {
									...
									for($a = 0; $a < $nb_loggers; $a ++)
										$modules ["log"] [$a]->log_hit ( $vhost, $remote_host, $remote_ip, $logged_user, trim ( $tmp_arr [0] ), $rq_err, $sent_content_length, $htreq_headers ["REFERER"], $htreq_headers ["USER-AGENT"] );
				...
				if ((! $conf ["global"] ["singleprocessmode"] [0]) || ($conf ["global"] ["servermode"] [0] == "inetd"))
					exit ( 0 );
