...
class Limit extends Unit implements IUnitTest {
	function testScalar(){
		$limit = new \Database\SQL\Parts\Limit(1,2);
		$this->assertEqual('LIMIT 1,2', (string)$limit,'test 1');
		
		$limit = new \Database\SQL\Parts\Limit(1);
	...
	function testArray(){
		$limit = new \Database\SQL\Parts\Limit(array(1,2));
		$this->assertEqual('LIMIT 1,2', (string)$limit,'test 1');
	
		$limit = new \Database\SQL\Parts\Limit(array(1));
		$this->assertEqual('LIMIT 1', (string)$limit,'test 2');
	...
	function testEmpty(){
		$limit = new \Database\SQL\Parts\Limit();
