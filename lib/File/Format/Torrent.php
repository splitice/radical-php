...
class Torrent {
	...
	*/
	...
	public function save ( $filename = null ) {
		return file_put_contents( is_null( $filename ) ? $this->info['name'] . '.torrent' : $filename, $this->encode( $this->Filtered() ), LOCK_EX );
	}
	...
	static private function decode_integer ( & $data ) {
		...
		if ( ! ctype_digit( $t = substr( $data, $start, $end-$start ) ) ){
