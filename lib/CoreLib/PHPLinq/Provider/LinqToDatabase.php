 ...
 * PHPLinq
 * 
 * Modified by SplitIce for CoreLib2
 *
...
class LinqToDatabase implements Interfaces\ILinqProvider {
	...
	/**
	 * Child ILinqProvider instances, used with join conditions
	 *
	 * @var ILinqProvider[]
	 */
	...
	/**
	 * PHPLinq\Adapter\Abstract instance
	 *
	 * @var PHPLinq\Adapter\Abstract
	 */
	...
	/**
	 * Query callback (static for all LinqToZendDb !)
	 * 
	...
	public static function handles($source) {
		return ($source instanceof Model\TableReferenceInstance);
	}
	...
	public function select($expression = null) {
		...
		if (count($this->_childProviders) > 0) {
			...
			foreach ($this->_childProviders as $provider) {
				if (get_class($provider) != __CLASS__) {
					throw new \Exception('Joins of ' . get_class($provider) . ' on ' . __CLASS__ . ' are not supported.');
				} else {
					// Supported! Add join
		...
		if (!is_null(self::$_queryCallback)) {
			...
			call_user_func(self::$_queryCallback, $zendDbSelect->toSQL());
		}
		...
		if (count($this->_childProviders) == 0) {
			// No join, easy!
			$results = $zendDbSelect->Query()->fetchAll();
			// Row class set? If so, convert all data!
			if (is_array($results)) {
				$rowClass = $this->_data->getClass();
				$tempResults = array();
				
				foreach ($results as $result) {
					$tempResult = call_user_func( array($rowClass, 'fromSQL'), $result );
			...
			$tempResults = $zendDbSelect->Query()->fetchAll();
	...
	public function where($expression) {
		$this->_where = !is_null($expression) ? new Expression($expression, $this->_from) : null;
		return $this;
	}
	...
	public function takeWhile($expression) {
		$this->_takeWhile = !is_null($expression) ? new Expression($expression, $this->_from) : null;
		return $this;
	}
	...
	public function skipWhile($expression) {
		$this->_skipWhile = !is_null($expression) ? new Expression($expression, $this->_from) : null;
		return $this;
	}
	...
	public function orderBy($expression, $comparer = null) {
		$this->_orderBy[0] = new OrderByExpression($expression, $this->_from, false, $comparer);
		return $this;
	}
	...
	public function orderByDescending($expression, $comparer = null) {
		$this->_orderBy[0] = new OrderByExpression($expression, $this->_from, true, $comparer);
		return $this;
	}
	...
	public function thenBy($expression, $comparer = null) {
		$this->_orderBy[] = new OrderByExpression($expression, $this->_from, false, $comparer);
		return $this;
	}
	...
	public function thenByDescending($expression, $comparer = null) {
		$this->_orderBy[] = new OrderByExpression($expression, $this->_from, true, $comparer);
		return $this;
	}
	...
	public function distinct($expression) {
		$this->_distinct = !is_null($expression) ? new Expression($expression, $this->_from) : null;
		return $this;
	}
	...
	public function ofType($type) {
		...
		// Assign expression
		$this->_ofType = new Expression($expression, $this->_from);
		return $this;
	}
	...
	public function join($name) {
		return new Initiator($name, $this);
	}
	...
	public function on($expression) {
		$this->_joinCondition = new Expression($expression, $this->_from);
	...
	public function aggregate($seed = 0, $expression) {
		$codeExpression = new Expression($expression);
	...
	/**
	 * Get query callback (static for all LinqToZendDb !)
	 * 
	 * Function accepting a string to which query strings can be logged.
	 *
	 * @return mixed
	...
	/**
	 * Set query callback (static for all LinqToZendDb !)
	 * 
	 * Function accepting a string to which query strings can be logged.
	 *
	 * @param mixed $value
